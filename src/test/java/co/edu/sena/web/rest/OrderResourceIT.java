...
class OrderResourceIT {
    ...
    public static Order createEntity(EntityManager em) {
        Order order = new Order().date(DEFAULT_DATE).status(DEFAULT_STATUS).total(DEFAULT_TOTAL);
        // Add required entity
    ...
    public static Order createUpdatedEntity(EntityManager em) {
        Order order = new Order().date(UPDATED_DATE).status(UPDATED_STATUS).total(UPDATED_TOTAL);
        // Add required entity
        OrderDetails orderDetails;
        if (TestUtil.findAll(em, OrderDetails.class).isEmpty()) {
            orderDetails = OrderDetailsResourceIT.createUpdatedEntity(em);
            em.persist(orderDetails);
            em.flush();
        } else {
            orderDetails = TestUtil.findAll(em, OrderDetails.class).get(0);
    ...
    void putNewOrder() throws Exception {
        ...
        updatedOrder.date(UPDATED_DATE).status(UPDATED_STATUS).total(UPDATED_TOTAL);
    ...
    void partialUpdateOrderWithPatch() throws Exception {
        ...
        partialUpdatedOrder.date(UPDATED_DATE).total(UPDATED_TOTAL);
    ...
    void fullUpdateOrderWithPatch() throws Exception {
        ...
        partialUpdatedOrder.date(UPDATED_DATE).status(UPDATED_STATUS).total(UPDATED_TOTAL);
