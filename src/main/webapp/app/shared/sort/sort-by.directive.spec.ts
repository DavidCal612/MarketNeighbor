...
@Component({
  template: `
    <table>
      <thead>
        <tr marketneighborSort [(predicate)]="predicate" [(ascending)]="ascending" (sortChange)="transition($event)">
          <th marketneighborSortBy="name">ID<fa-icon *ngIf="sortAllowed" [icon]="'sort'"></fa-icon></th>
        </tr>
      </thead>
    </table>
  `,
...
describe('Directive: SortByDirective', () => {
  ...
  it('should initialize predicate, order, icon when initial component predicate differs from column predicate', () => {
    ...
    // THEN
    expect(sortByDirective.marketneighborSortBy).toEqual('name');
